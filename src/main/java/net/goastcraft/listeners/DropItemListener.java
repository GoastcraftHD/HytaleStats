...
public class DropItemListener extends EntityEventSystem<EntityStore, DropItemEvent.Drop> {
    public DropItemListener() {
        super(DropItemEvent.Drop.class);
    }

    @Override
    public void handle(int index, @NonNullDecl ArchetypeChunk<EntityStore> archetypeChunk, @NonNullDecl Store<EntityStore> store, @NonNullDecl CommandBuffer<EntityStore> commandBuffer, @NonNullDecl DropItemEvent.Drop event) {
        ...
        if (saveData.containsKey(item.getBlockId())) {
            ...
            statData.incrementDropCount(event.getItemStack().getQuantity());
        } else {
